...
export class NepaliDatepickerComponent {
  ...
  formatter: DateFormatter = date => {
    ...
    return `${date.year}/${date.month}/${date.day}`; // Formatted Nepali Date
  };
  ...
  selectDate(day) {
    ...
    this.formatValue();
    this.close();
    // Convert Selecetd Date to Object to { year, month, day }
    ...
    //Convert BS to AD
    const selectedDatebs2ad = adbs.bs2ad(selectedDateFormat);
    const adFormatDate =
      selectedDatebs2ad.year +
      "-" +
      selectedDatebs2ad.month +
      "-" +
      selectedDatebs2ad.day;
    this.propagateChange(adFormatDate);
  }
  /**
   * @param {?} year
   * @return {?}
