...
export class LandingLedgerComponent implements OnInit {
  ...
  //Recursive Fuction to add ledgers without reload

  recursionTree() {
    const assets = [this.assetsData];
    console.log(assets);
    this.nestedChilds(assets);
  }
  ...
  nestedChilds(assets) {
    if (Array.isArray(assets) && assets.length) {
      for (const val of assets) {
        ...
        this.nestedChilds(child);
        this.ledgerDetails = child;
  ...
  getCashPartyResponse(responseEntity): void {
    ...
    console.log(responseEntity.GroupID);
    let addedObject = {
      Child: "",
    ...
    //Access using Recursive Function
    console.log(this.ledgerDetails);
    this.ledgerNode = this.ledgerDetails.filter((item) => {
      console.log(item.GroupOrLedgerNo);
      console.log(item.ID);
      console.log(addedObject.GroupOrLedgerNo);
      console.log(item.ID == addedObject.GroupOrLedgerNo);
      return item.ID == addedObject.GroupOrLedgerNo;
    });
    console.log(this.ledgerNode);
    for (const item of this.ledgerNode) {
      console.log(item.Child);
      item.Child.push(this.addedLedgerObject);
    }
    ...
    for (const item of groupArray) {
      const param = item.ID;
      this.ledgerService.getLedgersById(param).subscribe((res) => {
    ...
    this.ledgerService.getLedgerTreeView().subscribe(
      (response) => {
        ...
        localStorage.setItem(
          "LedgerTreeList",
          JSON.stringify(response.Entity.Tree)
        );
      },
      (error) => {
        this.treeViewLoading = false;
  ...
  getMajorGroups() {
    ...
    const groupArray = [];
    const branchArray = [];
    for (const item of ledgerTree) {
      //Display main branch details
      // console.log(item);
      branchArray.push(item);
      for (const item1 of item["Child"]) {
        //Display main group details
  ...
  spliceLedger(): void {
    ...
    for (const item of groupArray) {
      ...
      this.ledgerService.getLedgersById(param).subscribe((res) => {
        this.ledgerList[param] = res.Entity;
        this.ledgersOfGroups = res.Entity;
        this.ledgersOfGroups.splice(
          this.ledgersOfGroups.findIndex(
            (ledger) => ledger.ID == this.modalRef.content.selectedItem.ID
          ),
          1
        );
        // console.log(this.ledgersOfGroups);
      ...
      if (ledger["Title"] === "Assets") {
        for (const val of assets) {
          if (val.hasOwnProperty("Child") && val["Child"]) {
            //First Child Data i.e Current, Fixed Assets
  ...
  //Recursive Function for Basic COA
  ...
  editNewLedger(ledger): void {
    ...
    const initialState = {
      ...
      groupArrays: this.groupArray,
    };
    this.modalRef = this.modalService.show(AccountLedgerComponent, {
      initialState,
    });
  ...
  editNewGroup(group): void {
    ...
    const initialState = {
      selectedItem: this.selectedItem,
      branchArray: this.branchArray,
    };
    this.modalRef = this.modalService.show(AccountGroupComponent, {
      initialState,
    });
